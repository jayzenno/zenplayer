...
class SettingsStore(context: Context) {
    ...
    private fun loadPlayer() = PlayerSettings(
        ...
        rememberPosition = prefs.getBoolean("rememberPosition", true), hardwareAcceleration = prefs.getBoolean("hardware", true), deinterlacing = prefs.getBoolean("deinterlace", true),
        audioNormalization = prefs.getBoolean("audioNorm", false), showPlayerStats = prefs.getBoolean("stats", false), preferCatchupSibling = prefs.getBoolean("catchupSibling", true),
        catchupConfirmation = prefs.getBoolean("catchupConfirm", false), startCurrentProgrammeFromBeginning = prefs.getBoolean("programmeStart", true),
        liveEdgeOnChannelSwitch = prefs.getBoolean("liveEdge", true), enableTimeshiftWhenAvailable = prefs.getBoolean("timeshift", true)
    )
    ...
    private fun savePlayer(v: PlayerSettings) = prefs.edit().putString("playerEngine", v.engine.name).putString("externalPlayerPackage", v.externalPlayerPackage)
        ...
        .putBoolean("catchupSibling", v.preferCatchupSibling).putBoolean("catchupConfirm", v.catchupConfirmation).putBoolean("programmeStart", v.startCurrentProgrammeFromBeginning)
        .putBoolean("liveEdge", v.liveEdgeOnChannelSwitch).putBoolean("timeshift", v.enableTimeshiftWhenAvailable).apply()
    private fun saveEpg(v: EpgSettings) = prefs.edit().putString("epgPage", v.pageSize.name).putBoolean("epgPrevious", v.showPreviousHours).putBoolean("epgFuture", v.showFutureHours)
        .putBoolean("epgCenterNow", v.autoCenterOnNow).putBoolean("epgProgress", v.showProgrammeProgress).putBoolean("epgDescriptions", v.showDescriptions).putBoolean("epgCatchupBadge", v.showCatchupBadge).putBoolean("epgStartLabel", v.startFromBeginningLabel).apply()
    private fun saveRemote(v: RemoteSettings) = prefs.edit().putString("remoteUpDown", v.channelUpDown).putString("remoteLeftRight", v.leftRight).putString("remoteLong", v.longPressUpDown)
        .putString("remoteBack", v.backAction).putString("remoteOk", v.okAction).putBoolean("numeric", v.numericKeys).putBoolean("history", v.channelHistory)
        .putString("remoteLongLeft", v.longPressLeftRight).putString("remoteDoubleOk", v.doublePressOk).apply()
