...
class PlaylistStore(private val context: Context) {
    ...
    private fun loadChannels(): List<Channel> = runCatching { if (playlistFile.exists()) playlistFile.inputStream().bufferedReader(Charsets.UTF_8).use { M3uParser.parse(it) } else emptyList() }.getOrDefault(emptyList())
